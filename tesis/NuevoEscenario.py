...
def crearEscenarioNuevo(numEmpleados, numOrdenes, numDias, costoAns):
    ...
    stageFile = open("./pro1_request.json")
    stageData = json.load(stageFile)
    print(len(stageData['Tasks']))
    print(len(stageData['WorkCenters']))

    contador = 0
    while numOrdenes > contador:
        row = stageData['Tasks'][contador]
        ...
        while contadorDis < numOrdenes:
            comparar = stageData['Tasks'][contadorDis]
    ...
    empleadosData = stageData['WorkCenters']
    ...
    maxDia = []
    hoy = datetime.strptime(stageData['Tasks'][0]['DateSolutionlimit'],'%Y-%m-%dT%H:%M:%S')
    ...
    while contadorE < numEmpleados:
        ...
        while contadorDias < numDias:
            ...
            horasE.append((int(diaActual['EndTime'][0:2]) - int(diaActual['StartTime'][0:2]) - 1)-4)
            contadorDias += 1
...
crearEscenarioNuevo(10,375,6,0.05)
